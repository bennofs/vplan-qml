...
--------------------------------------------------------------------------------
-- Timetable type
type Timetable = UScheduleVoidConfig LessonLocation Int
...
lessonIx w d l = ix (lessonLocation w d l)
...
hs_scheduleMaxLessons ptr = fromInteger . fromMaybe 0 . imaxOnOf (from _DiscreteTime) lessonNr <$> deRefStablePtr ptr

hs_scheduleLoadFile :: CWString -> Int -> Ptr CWString -> IO (StablePtr Timetable)
hs_scheduleLoadFile cwstr strlen cwerror = handling _IOException (newCWString . handle >=> poke cwerror >=> const (newStablePtr blank)) $ do
